...
for ijkl_file in ijkl-sxytxz-*.asc; do
    # Extract the discretization pattern (e.g., "1-2-2")
    disc=$(echo "$ijkl_file" | sed -E 's/ijkl-sxytxz-([0-9-]+)\.asc/\1/')
    
    # Match the TW1 file with same discretization and Re number
    for twfile in TW1-2pi1piRe*-"$disc"-sol.asc; do
        # Skip if no matching TW1 file
        [[ -e "$twfile" ]] || continue
		...
		mkdir -p $base
        findsoln -eqb -zrel -T 10 -R "$Re" -sigma "$sigma_file" -symms sxytxz.asc -od "$base" "$ncfile"
